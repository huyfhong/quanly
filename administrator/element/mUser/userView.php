...
if ($keyword != '') {
    $list_user = $obj_user->UserSearch($keyword);
} else {
...
<div class="card shadow-sm">
    <div class="card-header bg-white py-3">
        <div class="d-flex justify-content-between align-items-center">
            <h5 class="mb-0 text-primary"><i class="bi bi-list-ul me-2"></i>Danh sách Người dùng</h5>
            <div class="d-flex align-items-center">
                <form class="d-flex me-3" role="search" method="get" action="./index.php">
                    <input type="hidden" name="req" value="userView">
                    <div class="input-group input-group-sm">
                        <input class="form-control" type="search" placeholder="Tìm kiếm user..." aria-label="Search" name="search" value="<?php echo htmlspecialchars($keyword); ?>">
                        <button class="btn btn-primary" type="submit"><i class="bi bi-search"></i></button>
                        <?php if ($keyword != ''): ?>
                            <a href="./index.php?req=userView" class="btn btn-outline-secondary" title="Xóa tìm kiếm"><i class="bi bi-x"></i></a>
                        <?php endif; ?>
                    </div>
                </form>
                <span class="badge bg-secondary rounded-pill"><?php echo $l; ?> User</span>
            </div>
        </div>
    </div>
    <div class="card-body p-0">
        ...
        <div class="table-responsive">
            <table class="table table-hover table-bordered mb-0">
                ...
                <tbody>
                    <?php foreach ($list_user as $v): ?>
                    <tr>
                        <td class="text-center fw-bold"><?php echo $v->iduser;?></td>
                        ...
                        <td class="text-center">
                            <div class="btn-group" role="group">
                                <a href="javascript:void(0);" onclick="confirmDelete('./element/mUser/userAct.php?reqact=deleteuser&iduser=<?php echo $v->iduser;?>')" 
        ...
        <?php else: ?>
            <div class="alert alert-info m-3 text-center">Chưa có dữ liệu người dùng.</div>
        <?php endif; ?>
    </div>
</div>
